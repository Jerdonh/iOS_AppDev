...
class EditGameViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        titleLabel.text = daGame.gameName
        minTextOutlet.text = String(daGame.minPlayers)
        maxTextOutlet.text = String(daGame.maxPlayers)
        materialsTextOutlet.text = daGame.materials
        rulesTextOutlet.text = daGame.rules
        daNewGame = daGame
        fetchGameData()
    ...
    @IBAction func DeleteGame(_ sender: Any) {
        ...
        for g in GamesArray
        {
            ...
            if(g.gameName == titleLabel.text)
            {
                //delete this one
                print("Game to be removed: ", GamesArray[i].gameName)
                GamesArray.remove(at: i)
                deleteGameCoreData()
                addBaseData()
                break
            }
    ...
    */
    ...
    func editGamesArray()
    {
        ...
        for g in GamesArray
        {
            if(g.gameName == titleLabel.text)
            {
                g.imageFileName = imageFileName
                g.minPlayers = Int(minTextOutlet.text!)!
                g.maxPlayers = Int(maxTextOutlet.text!)!
                g.materials = materialsTextOutlet.text
                g.rules = rulesTextOutlet.text
                //GamesArray.remove(at: i)
